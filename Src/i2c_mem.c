...
void eep_initial(uint16_t memAddr, uint8_t *data){
  ...
  SendDataTM1638();
	...
	25, 0,    // data[8,9];   K[0] пропорциональный коэфф. = 25
	...
	/* ---------------------- uint8_t ---------------------------------*/
	 60,      // data[26]; timer[0]  отключ.состояниe = 60 мин.
	  ...
	  5,      // data[30]; extOn[0]  смещение для ВКЛ. вспомогательного канала = 5 => 0.5 гр.C
	  5,      // data[31]; extOn[1]  смещение для ВКЛ. вспомогательного канала = 5 => 0.5 гр.C
	  2,      // data[32]; extOff[0] смещение для ОТКЛ. вспомогательного канала = 2 => 0.2 гр.C
	  2,      // data[33]; extOff[1] смещение для ОТКЛ. вспомогательного канала = 2 => 0.2 гр.C
   60,      // data[34]; air[0]    таймер проветривания пауза; = 60 мин.
	...
	160,      // data[46]; KoffCurr маштабный коэф. по току симистора  (160 для AC1010)
